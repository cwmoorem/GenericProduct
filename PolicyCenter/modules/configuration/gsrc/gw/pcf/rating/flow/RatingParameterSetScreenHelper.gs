...
class RatingParameterSetScreenHelper implements GenericRateBookFieldSearch {
  ...
  private static var mapFromCodeToType = {
      ...
      CalcRoutineParamName.TC_GENERICPRODUCTPARAMETER -> "acc.pc.lob.gp.rating.GPRatingParam"
  }
  ...
  function validateParameterSet(parameterSet: CalcRoutineParameterSet): String {
    ...
    var paramSetCount = Query.make(CalcRoutineParameterSet).compare("Code", Equals, parameterSet.Code).select().Count
    if (paramSetCount > expectedCount) {
      return displaykey.Validation.Rating.ParameterSet.DuplicateParameterSetCode
    }
  ...
  static function validateParameterType(parameterType: String): String {
    ...
    if (parameterType <> null) {
      ...
      if (classType == null) {
        return displaykey.Java.ValidationError.SystemTables.InvalidClassName(parameterType)
      }
    }
    return null
  }
