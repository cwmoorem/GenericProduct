...
//    data.copyStandardColumnsToActualColumns()
  ...
  private function rateCancellationShortRatePenalty(line : PersonalAutoLine) {
    var p = Prorater.forRounding(RoundingLevel, RoundingMode, TC_PRORATABYDAYS) // is this the right level and mode?
    var termSubTotal = CostDatas.where( \ cost -> cost.ProrationMethod != TC_FLAT).sum(\ c -> p.prorate(line.Branch.PeriodStart, p.findEndOfRatedTerm(line.Branch.PeriodStart, c.NumDaysInRatedTerm),
                                                      line.Branch.PeriodStart, line.Branch.PeriodEnd, c.ActualTermAmount))
    var subtotal = CostDatas.where( \ cost -> cost.ProrationMethod != TC_FLAT).sum(\ c -> c.ActualAmount)
    if (line.Branch.RefundCalcMethod == "shortrate" and subtotal != 0) {
      ...
      var rateRoutineParameterMap : Map<CalcRoutineParamName, Object> =
            {TC_POLICYLINE   ->PolicyLine,
             CalcRoutineParamName.TC_PRORATEDPREMIUMTOTAL -> termSubTotal - subtotal}
  ...
  private function getRateBook(refDate : Date, offeringCode : String) : RateBook {
    return RateBook.selectRateBook(refDate, Branch.RateAsOfDate, PolicyLine.PolicyLine.PatternCode,
                                   PolicyLine.BaseState, _ratingLevel, Branch.JobProcess typeis RenewalProcess, offeringCode, PolicyLine.Branch.UWCompany)
  }
}
