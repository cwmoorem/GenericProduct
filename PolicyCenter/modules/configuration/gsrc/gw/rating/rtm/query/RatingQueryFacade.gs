...
class RatingQueryFacade {
  ...
  static function matchRateBook(filter : RateBookQueryFilter, matchers : List<RateBookMatcher>) : List<RateBook> {
    ...
    // When working with Generic Product we need to take the Product code into account and make sure
    // that the candidates are the correct ones
    //
    if(filter.PolicyLine == "GPLine"){
      candidates = candidates.where( \ elt -> elt.GenericProductCode == filter.GenericProductCode)
      //
      // For generic product if there are no candidates we just get out and don't throw an exception
      //
