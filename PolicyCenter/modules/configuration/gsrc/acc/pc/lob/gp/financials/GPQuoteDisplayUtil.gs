...
class GPQuoteDisplayUtil {
  var _line : entity.GPGenericLine_Ext
  var _costsByCbl : Map<Coverable, List<GPCost_Ext>>
  ...
  var _trxnsByCbl : Map<Coverable, List<GPTransaction_Ext>>
  var _cblsWithTrxnsByType : Map<gw.lang.reflect.IType, List<Coverable>>
  
  construct(line : entity.GPGenericLine_Ext, forCosts : Boolean) {
    _line = line
  ...
  private function initCosts() {
    var allCosts = _line.VersionList.GPCosts.allVersionsFlat<GPCost_Ext>()
    _costsByCbl = allCosts.partition<Coverable>(\ cost -> cost.getSlice(cost.EffDate).OwningCoverable
                                                              .VersionList.AllVersionsUntyped.last() as Coverable )
  ...
  private function initTransactions() {
    ...
    var cblKeyToCblMap = allTrxns.map(\ trxn -> {
                                        // Map from Transaction to Coverable
                                        var cost = trxn.Cost as GPCost_Ext
    ...
    _trxnsByCbl = allTrxns.toList().partition<Coverable>(\ trxn -> {
                                                           var cost = trxn.Cost as GPCost_Ext
                                                           return cblKeyToCblMap.get(cost.getSlice(cost.EffDate).OwningCoverable.FixedId)
                                                         })
                           as  Map<Coverable, List<GPTransaction_Ext>>

    _cblsWithTrxnsByType = _trxnsByCbl.Keys.toList().partition(\ cbl -> (typeof cbl))
  }

  public function getCostsByCoverable() : Map<Coverable, List<GPCost_Ext>> {
    return _costsByCbl
  }
  ...
  public function getTrxnsByCoverable() : Map<Coverable, List<GPTransaction_Ext>> {
    return _trxnsByCbl
  }
  ...
  public static function CostDisplayName(cost : GPCost_Ext) : String {
    var cov = cost.Coverage
    if (cov != null) {
      if(cov.PolicyLine.Branch.Policy.GenericProduct != null){
        var name = (cov.OwningCoverable as GPGeneric_Ext).GenericSubproduct.GenericClauseMappings.firstWhere( \ elt -> elt.Column == cov.PatternCode).Name
        return name
      } else {
  ...
  public static function CoverableDisplayName(cbl : Coverable) : String {
    switch (typeof cbl) {
      case GPGeneric_Ext: return cbl.GenericSubproduct.Name + ": " + cbl.FixedId
